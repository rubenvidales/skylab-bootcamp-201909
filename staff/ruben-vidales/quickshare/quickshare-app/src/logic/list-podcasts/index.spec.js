...
const listPodcastsByRss = require('.')
const { random } = Math
...
describe('logic - list podcasts by rss id', () => {
    ...
    it('should succeed on correct user', async () => {
        const podcasts = await listPodcastsByRss(token, rssId)
        ...
        podcasts.forEach(podcast => {
            ...
            expect(podcast.rssChannel).toBeDefined()
            expect(podcast.rssChannel).toBeOfType('string')
            expect(podcast.rssChannel).toHaveLengthGreaterThan(0)
            expect(podcast.rssChannel).toEqual(rssId)
