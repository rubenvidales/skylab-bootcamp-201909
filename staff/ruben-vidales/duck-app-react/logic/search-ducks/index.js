function searchDucks(id, token, query, callback) {
    ...
    call('GET', token, 'https://skylabcoders.herokuapp.com/api/user/' + id, undefined, function (result) {
        ...
        call('GET', token, query ? 'https://duckling-api.herokuapp.com/api/search?q=' + query : 'https://duckling-api.herokuapp.com/api/search', undefined, function (result) {
            if (result.error)
                ...
                result.map(duck => { // normalize image url to image
                    ...
                    //Check if this duck is in favs array
                    duck.fav = favs.includes(duck.id)
                })
